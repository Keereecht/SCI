...
def run_top(text_info): # ใช้ text_widget จาก global
        ...
        for word in filtered_words:
            ...
            if rect_width < threshold_width or rect_height < threshold_height:
                ...
                reduced_rect = fitz.Rect(
                    original_rect.x0 - 0.8 * original_rect.width,
                    original_rect.y0 - 0.8 * original_rect.height,
                    original_rect.x1 + 0.8 * original_rect.width,
                    original_rect.y1 + 0.8 * original_rect.height
                )
            else:
    ...
    # กรองข้อมูลที่ไม่ startswith('IC') ออก
    data_top_no_ic = data_storage.data_top[~data_storage.data_top['Item_Description'].str.startswith('IC')]
    # บันทึกไฟล์ที่กรองข้อมูลที่ startswith('IC') ออกแล้ว
    file_path_top = os.path.join(data_storage.Main_folder, data_storage.selected_customer, data_storage.projectname, data_storage.projectname+'_TOP_SMT.csv')
    os.makedirs(os.path.dirname(file_path_top), exist_ok=True)
    data_top_no_ic.to_csv(file_path_top, index=False)

    data_ic_top = data_storage.data_top[data_storage.data_top['Item_Description'].str.startswith('IC')]
    file_path_ic_top = os.path.join(data_storage.Main_folder, data_storage.selected_customer, data_storage.projectname, data_storage.projectname+'_TOP_IC.csv')
    data_ic_top.to_csv(file_path_ic_top, index=False)
    end_time_top = time.time()
    elapsed_time_top = end_time_top - start_time_top

def run_top_ic():
    top_ic = data_storage.data_ic_top.shape
    output_path_ic_top = os.path.join(data_storage.Main_folder, data_storage.selected_customer, data_storage.projectname, data_storage.projectname+'_TOP_IC.pdf')
    os.makedirs(os.path.dirname(output_path_ic_top), exist_ok=True)
        # ตรวจสอบประเภทของ pdf_path
    if isinstance(data_storage.pdf_path, BytesIO):
        doc = fitz.Document(stream=data_storage.pdf_path)
    else:
    ...
    collected_values_list = []
    count_list = []
    for x in range(top_ic[0]):
        ref_des = data_storage.data_ic_top.iat[x, 3]
        ref_des_list = ref_des.split(",")
        filtered_words = [word for word in words if word[4] in ref_des_list]
        collected_values = []
        count = 0
        for word in filtered_words:
            ...
            color = (1, 0.65, 0.9)  # Default color light blue
            highlight.set_colors(stroke=color)
            highlight.update()
            collected_values.append(word[4])
        collected_values_list.append(",".join(collected_values))
        count_list.append(count)
    doc.save(output_path_ic_top)
...
def run_bot(text_info):
    ...
    data_bot_no_ic.to_csv(file_path_bot, index=False)

    data_ic_bot = data_storage.data_buttom[data_storage.data_buttom['Item_Description'].str.startswith('IC')]
    file_path_ic_bot = os.path.join(data_storage.Main_folder, data_storage.selected_customer, data_storage.projectname, data_storage.projectname+'_BOT_IC.csv')
    data_ic_bot.to_csv(file_path_ic_bot, index=False)

    end_time_bot = time.time()
    elapsed_time_bot = end_time_bot - start_time_bot

def run_bot_ic():
    bot_ic = data_storage.data_ic_bot.shape
    output_path_ic_bot = os.path.join(data_storage.Main_folder, data_storage.selected_customer, data_storage.projectname, data_storage.projectname+'_BOT_IC.pdf')
    os.makedirs(os.path.dirname(output_path_ic_bot), exist_ok=True)
        # ตรวจสอบประเภทของ pdf_path
    if isinstance(data_storage.pdf_path, BytesIO):
        doc = fitz.Document(stream=data_storage.pdf_path)
    else:
    ...
    collected_values_list = []
    count_list = []
    for x in range(bot_ic[0]):
        ref_des = data_storage.data_ic_bot.iat[x, 3]
        ref_des_list = ref_des.split(",")
        filtered_words = [word for word in words if word[4] in ref_des_list]
        collected_values = []
        count = 0
            ...
            if pattern.match(word):
                ...
                for word_item in words:
                    if word == word_item[4]:
                        ...
                        highlight.set_colors(stroke=color)
                        highlight.update()
                        print(word_item[4], "Actual_fond(EA)",loop_counter)
                        collected_values_list.append(word_item[4])
                        count_list.append(count)
                        writer.writerow([word_item[4], count])  # เขียนค่า word_item[4] และ count ลงในไฟล์ CSV
